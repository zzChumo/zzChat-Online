...
var homeText = "# XChat\n##### \n-----\n"+
"欢迎来到线圈附属小型即时聊天工具——XChat\n这是一个极简、小型的网页在线聊天工具，是HC的高度优化、升级版\n"+
...
function RequestNotifyPermission() {
	try {
				...
				if (result === "granted") {
					if (notifyPermissionExplained === 0) {
						pushMessage({
							cmd: "chat",
							nick: "*",
							text: "You can receive notification now.",
							time: null
						});
		...
		pushMessage({
			cmd: "chat",
			nick: "*",
			text: "Can't create notification.",
			time: null
		});
...
var COMMANDS = {
	list: function (args) {
		...
		if(args.text.length == 0) {
			pushMessage({ nick: '*', text: "----NO HISTORY MESSAGES----"})
		} else {
			pushMessage({ nick: '*', text: "----HISTORY MESSAGES----"})
		}
		var a_nicks = []
		for (var i=0; i<nicks.length; i+=1) {
			if (!(nicks[i].indexOf('afk') > 0)) {
	...
	unkill: function (args) {
		...
		pushMessage({ nick: '*', text: "Unkicked"+args.nick});
	},
	info: function (args) {
		args.nick = '*';
		pushMessage(args);
	...
	onlineAdd: function (args) {
		...
		if ($$('#joined-left').checked) {
			...
			*/
			pushMessage({ nick: '*', text: nick + " joined，from："+args.city });
		}
	},
	...
	onafkAdd: function (args) {
		...
		userAdd(nick);
		if ($$('#joined-left').checked) {
			pushMessage({ nick: '*', text: nick + " afk" });
		}
	},
	onafkRemove: function (args) {
		...
		userRemove(nick);
		if ($$('#joined-left').checked) {
			pushMessage({ nick: '*', text: nick + " active" });
		}
	},
	changenick: function (args) {
		var h5_mynick = document.querySelector("#mynick")
		h5_mynick.innerHTML = 'Your nickname：'+args.nick.split('#')[0]
	},
	video: function(args) {
		video_url = args.url
		console.log('video url:', video_url)
		if (video_url == 'none') {
			alert('There is no videos.Go and talk with mods.')
			return false;
		}
...
function pushMessage(args) {
	...
	if(!args.tag) {
		var html = ''
		if (args.text.indexOf('USERSENDVOICE_')>=0) {
		...
		args.text.forEach(v=> {
			...
			if (content.indexOf('USERSENDVOICE_')>=0) {
				...
				if (showHead) {
					...
					html = `<div class="message"><span class="nick"><a title="${v.time}">${v.nick}</a></span><p class="text"><p><audio controls><source src="https://xq.kzw.ink/oss/${content2.replace('USERSENDVOICE_', '').replace('static/', '')}" type="audio/mpeg"></audio></p></p></div>`+html
				}
			} else {
				if (showHead) {
